...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    ...
    <a href="{{ route('students.create') }}" class="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white px-5 py-2.5 rounded-xl font-semibold text-sm shadow-md transition">
...
@if ($students->isEmpty())
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-12 text-center">
        <div class="w-16 h-16 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg class="w-8 h-8 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
        </div>
        ...
        <a href="{{ route('students.create') }}" class="inline-block mt-4 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-xl text-sm font-semibold">Register Now</a>
    </div>
@else
    <!-- Desktop Table -->
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-green-50 border-b border-green-100">
                    <tr class="text-left text-green-800">
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($students as $student)
                        <tr class="hover:bg-green-50/50 transition">
                            ...
                            <td class="px-6 py-3"><span class="inline-flex px-2.5 py-1 rounded-full text-xs font-semibold bg-green-50 text-green-700 border border-green-200">{{ $student->program }}</span></td>
                            <td class="px-6 py-3 text-slate-600">{{ $student->year_level }}</td>
                            <td class="px-6 py-3 text-right">
                                <a href="{{ route('students.show', $student) }}" class="inline-flex items-center gap-1 bg-green-700 hover:bg-green-800 text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition">View</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        @foreach ($students as $student)
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 flex gap-4">
                ...
                <div class="flex-1 min-w-0">
                    <p class="font-mono text-xs text-green-600 font-semibold">{{ $student->student_id }}</p>
                    ...
                    <div class="flex items-center gap-2 mt-2 flex-wrap">
                        <span class="px-2 py-1 bg-green-50 text-green-700 border border-green-200 rounded-full text-xs font-medium">{{ $student->program }}</span>
                        <span class="text-xs text-slate-500">{{ $student->year_level }}</span>
                    </div>
                    <a href="{{ route('students.show', $student) }}" class="mt-3 inline-flex text-xs font-semibold text-white bg-green-700 px-3 py-1.5 rounded-lg">View Profile →</a>
                </div>
            </div>
        @endforeach
    </div>
